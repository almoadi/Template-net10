...
{
    /// <summary>
    /// Initializes every static facade (<c>Auth</c>, <c>Cache</c>, <c>Storage</c>, <c>Feature</c>) so each
    /// resolves its request-scoped service from the current <see cref="HttpContext"/>. Call once after
    /// the app is built. Prefer injecting the underlying interfaces; facades exist for Laravel-like ergonomics.
    /// </summary>
